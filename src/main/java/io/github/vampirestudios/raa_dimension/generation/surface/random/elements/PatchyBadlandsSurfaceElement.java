...
public class PatchyBadlandsSurfaceElement extends SurfaceElement {
    ...
    public void generate(Random random, ChunkAccess chunk, Biome biome, int x, int z, int height, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, int unknownNumber, long seed, TernarySurfaceConfig surfaceBlocks) {
        ...
        if (mesaNoise > 0.0D) {
            ...
            SurfaceBuilder.BADLANDS.generate(random, chunk, biome, x, z, height, noise, defaultBlock, defaultFluid, seaLevel, unknownNumber, seed, SurfaceBuilder.BADLANDS_CONFIG);
        } else {
    ...
    public ResourceLocation getType() {
        return new ResourceLocation("raa_dimensions", "patchy_badlands");
    }
