...
public class PortalHubFeature extends Feature<NoneFeatureConfiguration> {
    private final JsonConverter converter = new JsonConverter();
    private Map<String, JsonConverter.StructureValues> structures;

    public PortalHubFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
            ...
            if (decay <= 0 || !Rands.chance(14 - decay)) {
                switch (currBlockType) {
                    case "minecraft:stone_bricks":
                        WorldStructureManipulation.placeBlock(world, pos.offset(currBlockPos), Registry.BLOCK.getKey(theme.getBlock()).toString(), currBlockProp, 0);
                        break;
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        try {
            Resource path = Objects.requireNonNull(world.getServer()).getResourceManager().getResourceOrThrow(new ResourceLocation("raa_dimensions:structures/portal_hub/portal_hub.json"));
