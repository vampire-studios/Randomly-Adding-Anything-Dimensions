...
public class PatchyDesertSurfaceBuilder extends SurfaceSystem<TernarySurfaceConfig> {
    public static final PerlinSimplexNoise DESERT_NOISE = new PerlinSimplexNoise(new ChunkRandom(79L), IntStream.of(6, 0));
    ...
    public void generate(Random random, ChunkAccess chunk, Biome biome, int x, int z, int height, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, int unknownValue, long seed, TernarySurfaceConfig surfaceConfig) {
        double desertNoise = DESERT_NOISE.getValue(x * 0.049765625D, z * 0.049765625D, false);
        if (desertNoise > 0.0D) {
