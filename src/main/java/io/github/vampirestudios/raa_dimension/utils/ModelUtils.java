...
public class ModelUtils {
    ...
    public static void wall(ArtificeResourcePack.ClientResourcePackBuilder pack, Identifier id) {
        pack.addBlockState(id, blockStateBuilder -> {
            ...
            blockStateBuilder.multipartCase(aCase -> aCase.when("north", "low").apply(variant -> {
                variant.model(new Identifier(id.getNamespace(), "block/" + id.getPath() + "_side"));
                variant.uvlock(true);
            }));
            blockStateBuilder.multipartCase(aCase -> aCase.when("east", "low").apply(variant -> {
                variant.model(new Identifier(id.getNamespace(), "block/" + id.getPath() + "_side"));
                variant.uvlock(true);
                variant.rotationY(90);
            }));
            blockStateBuilder.multipartCase(aCase -> aCase.when("south", "low").apply(variant -> {
                variant.model(new Identifier(id.getNamespace(), "block/" + id.getPath() + "_side"));
                variant.uvlock(true);
                variant.rotationY(180);
            }));
            blockStateBuilder.multipartCase(aCase -> aCase.when("west", "low").apply(variant -> {
                variant.model(new Identifier(id.getNamespace(), "block/" + id.getPath() + "_side"));
                variant.uvlock(true);
                variant.rotationY(270);
            }));

            blockStateBuilder.multipartCase(aCase -> aCase.when("north", "tall").apply(variant -> {
                variant.model(new Identifier(id.getNamespace(), "block/" + id.getPath() + "_side_tall"));
                variant.uvlock(true);
            }));
            blockStateBuilder.multipartCase(aCase -> aCase.when("east", "tall").apply(variant -> {
                variant.model(new Identifier(id.getNamespace(), "block/" + id.getPath() + "_side_tall"));
                variant.uvlock(true);
                variant.rotationY(90);
            }));
            blockStateBuilder.multipartCase(aCase -> aCase.when("south", "tall").apply(variant -> {
                variant.model(new Identifier(id.getNamespace(), "block/" + id.getPath() + "_side_tall"));
                variant.uvlock(true);
                variant.rotationY(180);
            }));
            blockStateBuilder.multipartCase(aCase -> aCase.when("west", "tall").apply(variant -> {
                variant.model(new Identifier(id.getNamespace(), "block/" + id.getPath() + "_side_tall"));
                variant.uvlock(true);
                variant.rotationY(270);
            }));
